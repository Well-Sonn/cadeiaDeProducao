...
- StoreMain.java — ponto de entrada da loja
...
- TestClient.java — cliente de teste para comprar veículo
...
```bash
java -cp out loja.MockFactory 9000
```

3) Executar 3 lojas (exemplo):

```bash
java -cp out loja.StoreMain Loja1 10001 localhost 9000 5
java -cp out loja.StoreMain Loja2 10002 localhost 9000 5
java -cp out loja.StoreMain Loja3 10003 localhost 9000 5
```

4) Rodar um cliente de teste que compra um veículo de Loja1:

```bash
java -cp out loja.TestClient localhost 10001
```

Logs serão criados como store-<storeId>-recebimento.log e store-<storeId>-venda.log.

Protocolo resumido:
- Store <-> Factory: objetos Java serializáveis; a loja envia FactoryRequest(storeId, quantity) e a fábrica responde com objetos Vehicle seguidos por "END" ou "NONE".
